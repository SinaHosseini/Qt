        ...
        if game_status == "PvP":
            if player == 1:
                ...
                buttons[row][col].setStyleSheet(
                    "color: red; background-color: pink; font: 62pt G2 Rubber Stamp LET; border-radius: 8px;")
                player = 2

            elif player == 2:
                buttons[row][col].setText("O")
                buttons[row][col].setStyleSheet(
                    "color: #00007f; background-color: #9500d5; font: 62pt G2 Rubber Stamp LET; border-radius: 8px;")
                player = 1

        elif game_status == "Co-op":
            if player == 1:
                buttons[row][col].setText("X")
                buttons[row][col].setStyleSheet(
                    "color: red; background-color: pink; font: 62pt G2 Rubber Stamp LET; border-radius: 8px;")
                ...
                while True:
                    ...
                    if buttons[rand_row][rand_col].text() == "":
                        ...
                        buttons[rand_row][rand_col].setStyleSheet(
                            "color: #00007f; background-color: #9500d5; font: 62pt G2 Rubber Stamp LET; border-radius: 8px;")
                        player = 1
                        break
    except:
        msg_box = QMessageBox(text="Please select game mode.")
        msg_box.exec()
    ...
    for i in range(3):
        for j in range(3):
            ...
            buttons[i][j].setStyleSheet(
                "background-color: rgb(17, 151, 76); border-radius: 8px;")
            player = 1
